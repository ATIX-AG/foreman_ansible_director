...
export const crudSuccess = (
  ...
  message: __(_('Successfully %(operation)s %(resource)s!'), {
    // Construct past participle: "delete" -> "deleted"
...
export const crudError = (
  ...
  message: (
    <span>
      {__(_('%(plural)s occurred while %(operation)s %(resource)s!'), {
        plural: errors.length > 1 ? 'Multiple errors' : 'An error',
        // Construct present participle: "delete" -> "delet" -> "deleting"
        operation: operation.substring(0, operation.length - 1) + 'ing',
        resource: resourceIdDisplayNameMap[resource],
      })}
      <br />
      <Button variant={'control'} onClick={() => showAlertModal()}>{_('View details')}</Button>
